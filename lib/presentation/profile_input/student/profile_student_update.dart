...
import 'package:boilerplate/presentation/profile_input/student/profile_student_experience.dart';
...
class ProfileStudentUpdate extends StatefulWidget {
  const ProfileStudentUpdate({super.key});

  @override
  State<ProfileStudentUpdate> createState() => _ProfileStudentUpdateState();
}
...
class _ProfileStudentUpdateState extends State<ProfileStudentUpdate> {
  ...
  LanguageStudentList languages = LanguageStudentList(languageStudents: []);

  EducationList educations = EducationList(educations: []);
  ...
  void initState() {
    ...
    if (_profileStore.profile != null) {
      setState(() {
        skills = _profileStore.profile!.student!.skillSets!;
        languages = LanguageStudentList(
            languageStudents: _profileStore.profile!.student!.languages!);
        educations = EducationList(
            educations: _profileStore.profile!.student!.educations!);
      });
    }
  ...
  void getTechStacks() async {
    ...
    setState(() {
      ...
      techStacksValue = _profileStore.profile != null
          ? _profileStore.profile!.student!.techStack!.id!
          : techStacks.first.id;
    });
        ...
        children: [
          ElevatedButton(
            onPressed: () {
              ...
              Navigator.of(context)
                  .pushReplacement(MaterialPageRoute(builder: (context) {
                return const ProfileStudentExperince();
              }));
            },
            child: Text("Next",
                style: TextStyle(
